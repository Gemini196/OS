...
class JobsList {
 public:
  ...
  protected:
      ...
      bool is_stopped;
  public:
      // Noa please check if & is needed and if strcpy is needed
      JobEntry(int job_id, int pid, time_t time_inserted, string& command, bool is_stopped = false): job_id(job_id), pid(pid), time_inserted(time_insterted), command(command), is_stopped(stopped){};
      JobEntry(JobEntry& job){
          job_id = job.job_id;
          pid = job.pid;
          time_inserted = job.time_inserted;
