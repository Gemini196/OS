...
public:
    ...
    std::priority_queue<TimeoutEntry*, vector<TimeoutEntry*>, CmpEndTime> *timeout_queue;
    int curr_max_job_id;

    public:
    JobsList();
    ~JobsList();
    void addJob(string cmd_line, int pid, time_t time, bool is_stopped);
    void addTimeout(string cmd_line, int duration, pid_t pid , time_t time_inserted);
    bool isEmpty();
    void printJobsList();
    void killAllJobs();
    void deleteJobsList();
